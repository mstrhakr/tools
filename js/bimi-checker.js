(function () {
  ...
  function run() {
    ...
    fetch(API_BASE + '/api/bimi?domain=' + encodeURIComponent(domain) + '&selector=' + encodeURIComponent(selector || 'default'))
      .then(function (res) {
      ...
      .then(function (resp) {
        if (!resp.ok || resp.body.error) {
          throw new Error(resp.body.error || 'BIMI check failed');
        }
